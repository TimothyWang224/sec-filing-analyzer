...
from sec_filing_analyzer.utils.etl_logging import (
    get_etl_log_dir, get_recent_etl_runs, get_etl_run_stats, 
    get_etl_run_logs, generate_etl_report
)
...
with tab1:
    if not st.session_state.etl_runs:
        ...
        if 'end_time' in runs_df.columns:
            ...
            runs_df['duration_formatted'] = runs_df['duration'].apply(lambda x: f"{x:.2f}s" if pd.notnull(x) else "Running...")
