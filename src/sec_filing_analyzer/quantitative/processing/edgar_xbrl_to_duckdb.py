...
class EdgarXBRLToDuckDBExtractor:
    ...
    def __init__(self, db_path: Optional[str] = None, batch_size: int = 100):
        ...
        """
        self.db = OptimizedDuckDBStore(db_path=db_path, batch_size=batch_size)
        logger.info(f"Initialized Edgar XBRL to DuckDB extractor with database at {self.db.db_path}")
