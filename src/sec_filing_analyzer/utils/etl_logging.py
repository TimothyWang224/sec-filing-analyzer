...
def log_company_processing(
    ...
    if status == 'completed':
        ...
        etl_logger.error(
            f"Failed to process company {ticker}: {error}",
            extra=extra
        )
...
def log_filing_processing(
    ...
    if status == 'completed':
        ...
        etl_logger.info(
            f"Skipped filing {filing_id} ({company} {filing_type})",
            extra=extra
        )
    else:
        etl_logger.error(
...
def log_api_call(
    ...
    if success:
        etl_logger.debug(
            f"API call to {api_name} succeeded in {response_time:.2f}s",
            extra=extra
        )
    else:
        etl_logger.warning(
...
def log_embedding_stats(
    ...
    extra = {'run_id': run_id}
    etl_logger.info(
        f"Embedding stats: {tokens_used} tokens, {chunks_processed} chunks, "
