...
from sec_filing_analyzer.storage import OptimizedVectorStore, GraphStore
...
def test_advanced_filtering(store_path: str):
    ...
    for keywords, match_type in keywords_tests:
        ...
        if results:
            for i, result in enumerate(results[:2]):  # Show top 2
                logger.info(f"    Result {i+1}: {result['id']} - Vector: {result.get('vector_score', 0):.4f}, Keyword: {result.get('keyword_score', 0):.4f}")
    ...
    for i, result in enumerate(results['results']):
        ...
        if 'related_documents' in result and result['related_documents']:
            ...
            for j, related in enumerate(result['related_documents'][:3]):  # Show first 3
                logger.info(f"    Related {j+1}: {related.get('type', 'Unknown')} - {related.get('to_id', 'Unknown')}")
...
def main():
    ...
    parser.add_argument("--test", choices=["faiss", "filtering", "coordinated", "all"], default="all",
