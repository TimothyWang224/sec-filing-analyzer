...
with tab1:
    ...
    page = st.number_input("Page", min_value=1, max_value=max(1, (row_count + rows_per_page - 1) // rows_per_page), value=1)
...
with tab4:
    ...
    if selected_table in ["time_series_metrics", "financial_facts"]:
        ...
        def get_metrics(conn):
            metrics = conn.execute("SELECT DISTINCT metric_name FROM time_series_metrics ORDER BY metric_name").fetchall()
        ...
        with col1:
            selected_tickers = st.multiselect("Select Companies", tickers, default=["MSFT"] if "MSFT" in tickers else tickers[:1])
        
        with col2:
            selected_metrics = st.multiselect("Select Metrics", metrics, default=["Revenue"] if "Revenue" in metrics else metrics[:1])
