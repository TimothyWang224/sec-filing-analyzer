...
def compare_database_stats(old_db: OptimizedDuckDBStore, new_db: ImprovedDuckDBStore):
    ...
    try:
        ...
        # Count metrics
        old_stats['metrics_count'] = old_db.conn.execute("SELECT COUNT(DISTINCT metric_name) FROM time_series_metrics").fetchone()[0]
...
def main():
    ...
    parser.add_argument("--facts", nargs=2, metavar=('TICKER', 'ACCESSION'), help="Compare facts for a filing (specify ticker and accession number)")
    parser.add_argument("--time-series", nargs=2, metavar=('TICKER', 'METRIC'), help="Compare time series data (specify ticker and metric)")
    ...
    try:
        ...
        console.print(Panel(
            ...
            f"Old schema: {args.old_db}\n"
            f"New schema: {args.new_db}",
            box=box.ROUNDED
        ))
