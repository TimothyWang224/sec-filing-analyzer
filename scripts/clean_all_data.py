        ...
        try:
            ...
            if filing_count > 0:
                ...
                if not dry_run:
                    ...
                    confirm = input(f"Are you sure you want to delete {filing_count} filings? This cannot be undone! (yes/no): ")
                ...
                for i, row in enumerate(filings.itertuples()):
                    ...
                    logger.info(f"{'Simulating deletion' if dry_run else 'Deleting'} filing {i+1}/{filing_count}: {accession_number}")
                    
                    # Delete filing
                    result = lifecycle_manager.delete_filing(accession_number, dry_run=dry_run)
                    
                    if result["status"] == "success":
                        logger.info(f"Successfully {'simulated deletion' if dry_run else 'deleted'} filing {accession_number}")
                    else:
                        logger.warning(f"Issue {'simulating deletion' if dry_run else 'deleting'} filing {accession_number}: {result['status']}")
