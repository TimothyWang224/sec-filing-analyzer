...
def parse_timing_logs(log_file: Path) -> List[Dict[str, Any]]:
    ...
    timing_regex = re.compile(r'(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2},\d{3}) - (.*?) - (INFO|DEBUG|WARNING|ERROR) - TIMING: (.*?):(.*?) completed in (\d+\.\d+)s')
...
def find_bottlenecks(analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    return sorted(
        bottlenecks, 
        key=lambda x: (0 if x['severity'] == 'high' else 1, -x['avg_time'])
    )
...
def print_bottlenecks(bottlenecks: List[Dict[str, Any]]):
    ...
    for i, bottleneck in enumerate(bottlenecks[:3], 1):
        if bottleneck['category'] == 'llm':
            console.print(f"[yellow]{i}. Consider using a faster LLM model for {bottleneck['operation']} operations.[/yellow]")
            ...
            console.print(f"[yellow]{i}. Consider caching results from {bottleneck['operation']} to reduce API calls.[/yellow]")
        else:
            console.print(f"[yellow]{i}. Review the implementation of {bottleneck['operation']} in the {bottleneck['category']} category.[/yellow]")
