...
def test_lifecycle_manager():
    ...
    try:
        ...
        if "error" in company_filings:
            logger.error(
                f"Error getting filings for {ticker}: {company_filings['error']}"
            )
            return False
        ...
        if company_filings["filings"]:
            ...
            if "error" in filing_info:
                logger.error(
                    f"Error getting info for filing {accession_number}: {filing_info['error']}"
                )
            else:
                # Print filing info
                ...
                logger.info(
                    f"    Document Embedding Exists: {vector_store_info['document_embedding_exists']}"
                )
                logger.info(
                    f"    Chunk Embeddings Count: {vector_store_info['chunk_embeddings_count']}"
                )
                ...
                logger.info(
                    f"Testing deletion of filing {accession_number} (dry run)..."
                )
                deletion_result = lifecycle_manager.delete_filing(
                    accession_number, dry_run=True
                )

                if "error" in deletion_result:
                    logger.error(
                        f"Error deleting filing {accession_number}: {deletion_result['error']}"
                    )
                else:
                    ...
                    logger.info(
                        f"  Vector Store: {deletion_result['vector_store']['status']}"
                    )
                    logger.info(
                        f"  File System: {deletion_result['file_system']['status']}"
                    )
