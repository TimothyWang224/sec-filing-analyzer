    ...
    def add_section_type_classification(self):
        ...
        with self.driver.session(database=self.db) as session:
            ...
            for item in items:
                ...
                if record:
                    logger.info(
                        f"Updated {record['updated_count']} chunks with section_type '{section_type}' for item '{item}'"
                    )
            ...
            for record in result:
                ...
                if match:
                    ...
                    if update_result.single():
                        logger.info(
                            f"Inferred item '{item_number}' and set section_type '{section_type}' for chunk {chunk_id}"
                        )
        ...
        with self.driver.session(database=self.db) as session:
            ...
            for ticker in companies:
                ...
                for i in range(len(filings) - 1):
                    ...
                    if result.single():
                        logger.info(
                            f"Created temporal connection between filings {current_id} ({current_date}) and {next_id} ({next_date})"
                        )
            ...
            for company in companies:
                ...
                for term in search_terms:
                    ...
                    for chunk_id in chunk_ids:
                        ...
                        if result.single():
                            logger.debug(
                                f"Created MENTIONS relationship from chunk {chunk_id} to {company['ticker']} for term '{term}'"
                            )
