...
from scripts.demo import duckdb_store_extension  # noqa
...
def run_etl(ticker, years, test_mode=False):
    ...
    cmd = [
        "python",
        str(Path(project_root) / "scripts" / "demo" / "run_nvda_etl.py"),
        "--ticker", ticker
    ]
...
def query_revenue(ticker, year):
    ...
    cmd = [
        "python",
        str(Path(project_root) / "scripts" / "demo" / "query_revenue.py"),
        "--ticker", ticker,
        "--year", str(year)
    ]
...
def main():
    ...
    selected_metrics = st.sidebar.multiselect(
        "Select Metrics",
        available_metrics,
        default=["Revenue", "NetIncome"] if "Revenue" in available_metrics and "NetIncome" in available_metrics else available_metrics[:2]
    )
    ...
    with col1:
        ...
        if 'df' in st.session_state and not st.session_state.df.empty:
            ...
            st.markdown('<div class="info-box">Select a company and metrics, then click "Query Financial Data" to see the visualization.</div>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        if st.session_state.etl_complete:
            st.markdown(f'<div class="success-box">ETL process for {st.session_state.selected_company} completed successfully!</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="warning-box">ETL process not yet run. Click "Run ETL Process" to start.</div>', unsafe_allow_html=True)
