    ...
    for tool_name, tool_info in tools.items():
        ...
        for query_type, param_model in tool_spec.input_schema.items():
            ...
            try:
                ...
                print(
                    f"  Parameters: {json.dumps({'query_type': query_type, 'parameters': test_params}, indent=2)}"
                )

                result = await environment.execute_action(
                    {
                        "tool": tool_name,
                        "args": {"query_type": query_type, "parameters": test_params},
                    }
                )
                ...
                if isinstance(result, dict) and "output_key" in result:
                    ...
                    assert result["output_key"] == tool_spec.output_key, (
                        "Output key mismatch"
                    )
...
def create_test_parameters(tool_name, query_type):
    ...
    if tool_name == "sec_financial_data":
        ...
        if query_type == "semantic_search":
            return {
                "query": "What are the risks related to supply chain?",
                "companies": ["AAPL"],
                "top_k": 5,
            }
