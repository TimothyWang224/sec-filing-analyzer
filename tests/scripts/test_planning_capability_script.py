...
from src.capabilities import LoggingCapability, PlanningCapability
...
async def test_planning_with_coordinator():
    ...
    result = await agent.run(
        "Analyze Apple's financial performance over the last 3 years, focusing on revenue growth, profit margins, and compare it to Microsoft"
    )
    ...
    if plan_items:
        ...
        for step in plan.get("steps", []):
            print(
                f"  {step.get('step_id', '?')}. {step.get('description', 'No description')} - Status: {step.get('status', 'unknown')}"
            )
