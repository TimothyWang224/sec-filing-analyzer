...
async def run_phase_transition_demo():
    ...
    for i in range(3):  # Exceeds max_planning_iterations
        ...
        print(
            f"Iteration {agent.state.current_iteration}, Planning iterations: {agent.state.phase_iterations['planning']}"
        )
    ...
    for i in range(4):  # Exceeds max_execution_iterations
        agent.state.increment_iteration()
        print(
            f"Iteration {agent.state.current_iteration}, Execution iterations: {agent.state.phase_iterations['execution']}"
        )
        should_terminate = agent.should_terminate()
        print(f"Should terminate: {should_terminate}")
    ...
    for i in range(2):  # Exceeds max_refinement_iterations
        agent.state.increment_iteration()
        print(
            f"Iteration {agent.state.current_iteration}, Refinement iterations: {agent.state.phase_iterations['refinement']}"
        )
        should_terminate = agent.should_terminate()
        print(f"Should terminate: {should_terminate}")
