...
def fix_database_schema(db_path="data/db_backup/improved_financial_data.duckdb"):
    ...
    try:
        ...
        fiscal_period_exists = conn.execute("SELECT COUNT(*) FROM information_schema.columns WHERE table_name = 'filings' AND column_name = 'fiscal_period'").fetchone()[0] > 0
        
        if fiscal_period_exists:
            # Check the data type of the fiscal_period column
            fiscal_period_type = conn.execute("SELECT data_type FROM information_schema.columns WHERE table_name = 'filings' AND column_name = 'fiscal_period'").fetchone()[0]
